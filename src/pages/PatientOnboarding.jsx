...
const PatientRegistrationForm = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [businessProviderRes, insuranceRes, clientStaffRes] =
          await Promise.all([
        ...
        setClientStaffs(
          clientStaffRes.data.adminUsers.filter(
            (user) => user.role === "ClientStaff"
          )
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataWithFiles.append("primaryInsuranceCard", formData.primaryInsuranceCard);
      formDataWithFiles.append("secondaryInsuranceCard", formData.secondaryInsuranceCard);
      formDataWithFiles.append("idCard", formData.idCard);

      // Append other form data fields to formDataWithFiles as well
      Object.keys(formData).forEach((key) => {
        if (key !== "primaryInsuranceCard" && key !== "secondaryInsuranceCard" && key !== "idCard") {
          formDataWithFiles.append(key, formData[key]);
        }
      });

      const response = await axios.post(
        "/v1/api/patient/create-new-patient",
        formDataWithFiles,
        {
          headers: {
      ...
      <form className="form" onSubmit={handleSubmit}>
        ...
        <div className="section">
          ...
          <input
            ...
            accept="image/*"
            onChange={(e) => setFormData({ ...formData, primaryInsuranceCard: e.target.files[0] })}
          />
        </div>
        <div className="input-field">
          ...
          <input
            ...
            accept="image/*"
            onChange={(e) => setFormData({ ...formData, secondaryInsuranceCard: e.target.files[0] })}
          />
        </div>
